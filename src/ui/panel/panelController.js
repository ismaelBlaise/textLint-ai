...
exports.showCorrectionPanel = showCorrectionPanel;
exports.dispose = dispose;
const vscode = __importStar(require("vscode"));
let panel = undefined;
let currentCorrections = [];
function showCorrectionPanel(corrections, stats) {
    currentCorrections = corrections;
    const column = vscode.ViewColumn.Beside;
    if (!panel) {
        panel = vscode.window.createWebviewPanel("textlintAI", "TextLint AI - Corrections", column, {
            enableScripts: true,
...
async function applySingleCorrection(index) {
    ...
    await editor.edit((editBuilder) => {
        editBuilder.replace(new vscode.Range(correction.start, correction.end), correction.text);
    });
...
function ignoreSingleCorrection(index) {
    currentCorrections.splice(index, 1);
    if (panel) {
        panel.webview.html = getHtml(currentCorrections);
    }
...
async function exportReport(corrections) {
...
function generateMarkdownReport(corrections) {
    ...
    corrections.forEach((c, i) => {
        ...
        report += `**Position:** Ligne ${c.start.line + 1}, Colonne ${c.start.character + 1}\n`;
        if (c.confidence) {
...
function getHtml(corrections, stats) {
          ...
          <ul>
            ${c.changes
              ...
              <li>
                ...
                <code>${escapeHtml(ch.original)}</code> → <code>${escapeHtml(ch.corrected)}</code>
                ${ch.explanation
      ...
      `
        ...
        <div class="correction-item" data-index="${index}">
          ...
          <div class="correction-actions">
            ...
            <button class="btn btn-copy" onclick="copyCorrection('${escapeHtml(c.text).replace(/'/g, "\\'")}')">
              📋 Copier
            </button>
          </div>
        </div>
